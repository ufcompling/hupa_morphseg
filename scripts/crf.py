...
"""
 i think my binary where B is in front of all morphemes
 sliding window, hit B append then shrink window
"""
    ...
    for split in ['random', 'adversarial']:#, 'morph', 'length']: ## how to split residual data
        for i in range(n_folds):
            for split_n in ['1', '2', '3']:
                train_file = 'data/' + lg + '/' + method + "_" + n + '/test_' + str(int(test_proportion * 100)) + '/' + lg + '_train_' + str(i + 1) + '_' + split + '_' + split_n + '.tgt'	
                dev_file = 'data/' + lg + '/' + method + "_" + n + '/test_' + str(int(test_proportion * 100)) + '/' + lg + '_dev_' + str(i + 1) + '_' + split + '_' + split_n + '.tgt'
                test_file = 'data/' + lg + '/' + method + "_" + n + '/test_' + str(int(test_proportion * 100)) + '/' + lg + '_test_' + str(i + 1) + '.tgt'

                dev_filename = dev_file.split('.')[0]		
                dev_pred_file = 'preds/' + lg + '_' + n + '/' + method + '/test_' + str(int(test_proportion * 100)) + '/' + model + '/' + dev_filename.split('/')[-1] + '_1.pred' ## train one random seed for statistcal models

                test_filename = test_file.split('.')[0]
                test_pred_file = 'preds/' + lg + '_' + n + '/' + method + '/test_' + str(int(test_proportion * 100)) + '/' + model + '/' + test_filename.split('/')[-1] + '_' + split + '_' + split_n + '_1.pred'
                ...
                if os.path.isfile(train_file) and os.path.isfile(dev_file) and os.path.isfile(test_file):
                    ...
                    if ((not os.path.isfile(dev_pred_file)) or os.stat(dev_pred_file).st_size == 0) or ((not os.path.isfile(test_pred_file)) or os.stat(test_pred_file).st_size == 0):
                        ...
                        Y_dev_predict, Y_test_predict = build('models/' + lg + '_' + n + '/' + method + '/test_' + str(int(test_proportion * 100)) + '/' + model + '/' + model_filename, dictionaries, train_words, dev_words, test_words, args.d, args.e, args.i)
